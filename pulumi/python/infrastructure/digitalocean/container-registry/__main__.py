...
registry_name, err = external_process.run(
    cmd=registry_name_query_cmd, suppress_error=True)
...
if not err and registry_name and not registry_name.startswith('shared-global-container-registry-'):
    pulumi.log.info(
        f'Using already existing global Digital Ocean container registry: {registry_name}')
    container_registry = docean.ContainerRegistry.get(
        registry_name, id=registry_name)
else:
